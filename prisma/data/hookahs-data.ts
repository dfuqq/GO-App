import { Business, Images } from './types';

export const hookahsData: Business[] = [
	{
		slug: 'aloft',
		name: 'Art Loft',
		subtitle: 'Лаунж-бар на Ленина 26',
		description:
			'"Art Loft" можно смело назвать одной из лучших кальянных города.\nХорошая кухня, освежающие напитки и качественные, дымные кальяны — вот что ждёт Вас в этом заведении. Приветливый и опытный персонал всегда поможет Вам выбрать наилучший табак и подготовит кальян для Вашего комфортного отдыха.\nОтметим также классный дизайн заведения, наличие постоянных акций, удобное расположение, а ещё частые вечеринки под уикенд.\n\nПосетить заведение можно по адресу ниже.',
		descriptionHeader: 'В центре внимания',
		address: 'Ленина, 26',
		hours: '12:00 - 02:00 (Пт: 12:00 - 04:00, Сб: 16:00 - 04:00, Вс: 16:00 - 02:00)',
		priceFrom: '700',
		priceTo: '1000',
		phone: '68-01-17',
		logo: 'https://sun9-24.userapi.com/DipVCamBoW3mMKHq4aOw-JRz1o7EXRIMxxEqaw/i3gJTDcW6Ts.jpg',
		social: '@artloft_hookah_club',
		socialHref: 'https://instagram.com/artloft_hookah_club',
		categoryId: 3,
		// NOTE: Credits
		// <Footer>
		// 	За фото спасибо:{' '}
		// 	<Link href='https://vk.com/antoninkin' target='_blank'>
		// 		Антон Инкин
		// 	</Link>
		// 	.
		// </Footer>
	},
	{
		slug: 'brutal',
		name: 'Brutal Avenue',
		subtitle: 'Сеть кальянных',
		description:
			'"Brutal" в названии заведения точно не просто слово: это ты понимаешь, как только попадаешь сюда.\nДизайн кальянной отсылает нас к андерграунд культуре, но есть в нём и ещё одна интересная деталь — он передаёт внутренний взгляд создателей на кальянную культуру современности.\nПотому "Brutal Avenue" и пытается разрушить повседневность и стандарты, показывая себя с лучших сторон. Он цепляет своей атмосферой, доступными ценами и высоким качеством обслуживания. За приятную цену Вы сможете провести полноценный вечер с кальянами различного табака, настольными и консольными играми, безалкогольными напитками и вкусными закусками.\n\nВ "Brutal Avenue" делают всё для Вашего комфорта. Проверьте сами.',
		descriptionHeader: 'Андерграунд-лофт',
		address: 'пр. Набережный, 52\nРеспублики, 85',
		hours: '14:00 - 02:00 (Пт, Сб - до 04:00)',
		priceFrom: '600',
		priceTo: '1200',
		phone: '29-99-93 (пр. Набережный, 52)\n99-99-18 (Республики, 85)',
		logo: 'https://sun9-22.userapi.com/c-5HUHWFnr4RFjlbtEAh4ZdEjpxiVOlYcw7Y8A/sg67KruP_u0.jpg',
		social: '@brutal_avenue_hookah_club',
		socialHref: 'https://instagram.com/brutal_avenue_hookah_club',
		categoryId: 3,
	},
	{
		slug: 'bsb',
		name: 'Baker Street 221B',
		subtitle: 'Лаунж-бар на Университетской 29',
		description:
			'Сразу заявим: заведение не совсем про Шерлока. Но, фанаты сериала, не стоит огорчаться — "Baker Street" ещё способны удивить!\nДа, здесь есть отсылки на сериал. Очень красивые отсылки, к слову.\nНо не отсылками славится данное заведение, а первоклассной кухней с кальянами. Говоря честно, мы даже не можем выбрать, что лучше: "Baker" тут хорош везде. Персонал отзывчивый и всегда готов помочь с кальяном, кухня очень вкусная и недорогая, так ещё и часто проводятся вечеринки.\n\nСловом, нас сразили наповал. Оценить самому можно по адресу ниже.',
		descriptionHeader: 'Элементарно, Ватсон',
		address: 'Университетская, 29',
		hours: '12:00 - 02:00 (Сб, Вс - до 04:00)',
		priceFrom: '1000',
		priceTo: '1500',
		phone: '44-72-27',
		logo: 'https://sun9-24.userapi.com/_ktY0WfyeGU2jPBl35bn_7j5mQqIi4UtxqN1PA/Pw1E_GIIbKA.jpg',
		social: '@bakerstreet_universitetskaya',
		socialHref: 'https://instagram.com/bakerstreet_universitetskaya',
		categoryId: 3,
		// NOTE: Credits
		// <Footer>
		// 	За фото спасибо:{' '}
		// 	<Link href='https://vk.com/id391241331' target='_blank'>
		// 		Кристина Кочамай
		// 	</Link>
		// 	,
		// 	<br />
		// 	<Link href='https://vk.com/antoninkin' target='_blank'>
		// 		Антон Инкин
		// 	</Link>
		// 	.
		// </Footer>
	},
	{
		slug: 'hplace',
		name: 'HookahPlace',
		subtitle: 'Лаунж-бар на 30 Лет Победы 50',
		description:
			'"HookahPlace" — сеть мировых кальянных. И да, одно из заведений этой мировой сети находится в Сургуте.\nБудучи одними из первооткрывателей кальянного бизнеса в России, заведение задаёт тренд всей кальянной культуры страны, предлагая своему гостю премиальное обслуживанье за вполне демократичные цены. В "HookahPlace" Вы найдёте табак и кальяны исключительно мировых брендов, а опытные мастера, проходящие обучение в академии компании, грамотно сделают его специально по Вашим предпочтениям.\n\nВ дополнение Вы получаете доступ к настольным и видео-играм, прохладительным напиткам и блюдам из бара. А ещё велик шанс, что Вы попадёте на какой-нибудь тренинг или мастер-класс: они проводятся регулярно, позволяя поглубже окунуться в кальянный мир.\n\nСловом, место обязательно для посещения. Всю информацию Вы можете найти ниже.',
		descriptionHeader: 'Мировой представитель',
		address: '30 Лет Победы, 50',
		hours: '12:00 - 02:00 (Пт: 12:00 - 03:00, Сб: 15:00 - 03:00, Вс: 15:00 - 01:00)',
		priceFrom: '900',
		priceTo: '1300',
		phone: '71-27-77',
		logo: 'https://sun9-9.userapi.com/BTn5r1FlTColigjzEbCP4iSzrJVpOjx_5W5m4g/5XyZrK9JiOk.jpg',
		social: '@hookahplace_surgut',
		socialHref: 'https://instagram.com/hookahplace_surgut',
		categoryId: 3,
		// NOTE: Credits
		// <Footer>
		// 	За фото спасибо:{' '}
		// 	<Link href='https://vk.com/antoninkin' target='_blank'>
		// 		Антон Инкин
		// 	</Link>
		// 	.
		// </Footer>
	},
	{
		slug: 'jin',
		name: 'Jin',
		subtitle: 'Лаунж-бар на Энергетиков 7',
		description:
			'В городской суете так хочется спокойствия и тишины, что порой не знаешь, куда себя деть.\n"Jin" — тот самый приют, то самое уютное и тихое место, в котором можно отдохнуть, не обращая внимания на всё вокруг.\n\nПросто забирайте друзей с собой, приходите и усаживайтесь поудобнее в одной из зон с игровыми приставками. Не забудьте заказать себе поесть из мировой кухни, взять напитки из бара (от чая до спиртного) и, конечно, забить качественный кальян — подобрать микс Вам помогут опытные мастера.\n\nУйти от реальности можно по адресу ниже.',
		descriptionHeader: 'Долгожданное место',
		address: 'Энергетиков, 7',
		hours: '15:00 - 02:00 (Пт, Сб - до 05:00)',
		priceFrom: '1000',
		priceTo: '1500',
		phone: '97-26-27',
		logo: 'https://sun9-25.userapi.com/__dHib63Yiq6yr4iF8_cV1uQ19BFkLVcCvp9Yw/0iloCqNBZbM.jpg',
		social: '@jin_bar_',
		socialHref: 'https://instagram.com/jin_bar_',
		categoryId: 3,
	},
	{
		slug: 'light',
		name: 'Light Дым',
		subtitle: 'Кальянная на 30 Лет Победы 37',
		description:
			'"Light Дым" великолепен. И это не преувеличение!\n\nВы не найдёте места лучше, чем этот лаунж-бар. Широкий ассортимент блюд мировой кухни, отличные напитки из бара, игры, а самое главное — великолепные кальяны!\n\nЗнающий своё дело персонал с удовольствием будет обслуживать Вас в течение всего времени визита, делая Ваш визит приятным и незабываемым.\n\nВперёд отдыхать! Отлично провести вечер можно по адресу ниже.',
		descriptionHeader: 'Эталон качества',
		address: '30 Лет Победы, 37',
		hours: '12:00 - 00:00 (Пт, Сб - до 03:00)',
		priceFrom: '900',
		priceTo: '1300',
		phone: '91-42-40',
		logo: 'https://sun9-56.userapi.com/1QiJWYgY-A95PzKoKOvspaO5OthskjzhxlpC9Q/g84sUBGhIyo.jpg',
		social: '@lightdym',
		socialHref: 'https://instagram.com/lightdym',
		categoryId: 3,
	},
	{
		slug: 'mint',
		name: 'Мята',
		subtitle: 'Лаунж-бар на Мира 47/2',
		description:
			'Вы наверянка знаете про "Мяту", даже если не курите кальян.\n\nВсё просто. "Мята" — излюбленное место сургутской молодёжи. Это всероссийская сеть кальянных со своими принципами работы, исключением которых не стал и Сургут. Её полюбили за отличный сервис, приятный дизайн и широкий ассортимент кальянов.\n\nХотите "Darkside"? Не вопрос. "Spectrum"? Да, и он есть. "Musthave"? Прямо сейчас.\n\nРасполагайтесь поудобнее в закрытой зоне или зоне с приставками, выбирайте себе чай, коктейль или даже алкоголь в дополнение к Вашему кальяну, и получайте удовольствие.\n\nДа, это "Мята". Адрес ниже.',
		descriptionHeader: 'Мятный дым',
		address: 'Мира, 47/2',
		hours: '14:00 - 02:00 (Пт, Сб - до 04:00)',
		priceFrom: '700',
		priceTo: '1000',
		phone: '+7 (996) 329-07-04',
		logo: 'https://sun9-14.userapi.com/lx3XL8VEbH38oRtZoFDbeZyRuoE50UDUKeZ3BA/fS1oGPsdBLo.jpg',
		social: '@myata_surgut',
		socialHref: 'https://instagram.com/myata_surgut',
		categoryId: 3,
		// NOTE: Credits
		// Footer>
		// 	За фото спасибо:{' '}
		// 	<Link href='https://vk.com/antoninkin' target='_blank'>
		// 		Антон Инкин
		// 	</Link>
		// 	.
		// </Footer>
	},
	{
		slug: 'mrmld',
		name: 'Мармелад',
		subtitle: 'Лаунж-бар на Иосифа Каролинского 4',
		description:
			'Вы когда-нибудь представляли себе идеальную вечеринку? Забудьте и придите в "Marmelad".\n\nВсё начинается с дизайна, и он тут на высшем уровне. Обилие дыма и неона сразу погружает в атмосферу светских хроник, позволяя отключиться от всех проблем и сосредоточиться лишь на отдыхе.\n\nУже заняли своё место? Расслабьтесь и подготовьтесь сделать заказ: для Вас лучшие кальянные мастера могут сделать забивки мировых брендов разной крепкости, а в дополнение к отличному кальяну предложат коктейли из бара.\n\nПокурили? Тогда вперёд на танцпол к остальным гостям! Музыка от приглашённых DJ точно не отпустит Вас, пока не будет проиграна последняя пластинка.\n\nЭта ночь Вам запомнится надолго. Доверьтесь нам — придите по адресу ниже.',
		descriptionHeader: 'Образец вечеринок',
		address: 'Иосифа Каролинского, 4',
		hours: '12:00 - 02:00 (Пт, Сб - до 04:00)',
		priceFrom: '1000',
		priceTo: '1500',
		phone: '+7 (922) 419‒08‒36',
		logo: 'https://sun9-76.userapi.com/-_FW6QnLQImd0CWCaWC_1YB6q1GGBmV9UPdHIg/KOz5AA_SmdA.jpg',
		social: '@marmelad_surgut',
		socialHref: 'https://instagram.com/marmelad_surgut',
		categoryId: 3,
		//   NOTE: Credits
		// <Footer>
		// 			За фото спасибо:
		// 			<br />
		// 			<Link href='https://vk.com/antoninkin' target='_blank'>
		// 				Антон Инкин
		// 			</Link>
		// 			,&#13;
		// 			<Link href='https://vk.com/evgpro86' target='_blank'>
		// 				{' '}
		// 				Евгений Кирсанов
		// 			</Link>
		// 			,&#13;
		// 			<br />
		// 			<Link href='https://vk.com/mm_ivanovna' target='_blank'>
		// 				Мария Маноли
		// 			</Link>
		// 		</Footer>
	},
	{
		slug: 'office',
		name: 'The OFFICE Nargilia',
		subtitle: 'Кальянная на Энгельса 11',
		description:
			'"The OFFICE" работает в России с 2009 года, и за это время компания создала чёткие принципы работы своих заведений.\n\nПервым делом Вы отметите дизайн: Hi-Tech интерьер, выполненный в смеси типичного офисного кабинета и космического корабля. Он сразу даёт понять, что Вы пришли сюда лишь отдыхать, вырываясь из будничной суеты.\n\nПриходите с компанией. Здесь каждый найдёт себе занятие за большими столиками для общения, игровыми приставками или настольными играми.\n\nК Вашим услугам работает бар с безалкогольными напитками, а также часто проводятся мастер-классы от кальянных мастеров, рассказывающих больше о кальянах.\n\nКажется, время оторваться. Вас ждут в "Офисе" по адресу ниже.',
		descriptionHeader: 'Футуризм во всём',
		address: 'Энгельса, 11',
		hours: '15:00 - 03:00 (Пт, Сб - до 06:00)',
		priceFrom: '700',
		priceTo: '1300',
		phone: '64-06-88',
		logo: 'https://sun9-1.userapi.com/uPuAm_M-c9ko2ZJr3c_AN4rJ3liJabAjiC8Kow/T3BrfMOY1WA.jpg',
		social: '@the_office_surgut',
		socialHref: 'https://instagram.com/the_office_surgut',
		categoryId: 3,
	},
	{
		slug: 'pbar',
		name: 'ПароБар',
		subtitle: 'Вейп-бар на Игоря Киртбая 18',
		description:
			'Доводилось ли Вам бывать в вейп-барах? Если нет, тогда Вам определённо стоит попробовать данный формат.\n\n"ПароБар" позволяет Вам выбрать себе товар из огромного числа табачной продукции и протестировать его прямо в стенах магазина. А выбор есть: в бар поставляются эксклюзивные компоненты для вейпов и кальянов.\n\nНо также здесь можно провести время ничего не приобретая, просто попарив за столом в сопровождении напитков из бара. Здесь рады каждому посетителю, и каждый работник поддержит беседу с Вами.\n\nМеньше слов, больше пара! Адрес заведения указан ниже.',
		descriptionHeader: 'Отдел тестирования',
		address: 'Игоря Киртбая, 18',
		hours: '11:00 - 23:00',
		priceFrom: '700',
		priceTo: '1200',
		phone: '37-47-07',
		logo: 'https://sun9-48.userapi.com/GCsVQ0QjfESsMDNWhjca0YBT_aaBCTqVBMEVUQ/2Py4Zx5kxMk.jpg',
		social: '@parobarshop',
		socialHref: 'https://instagram.com/parobarshop',
		categoryId: 3,
	},
];

export const hookahsImagesData: Images[] = [
	{
		src: 'https://sun9-4.userapi.com/Z3xbwfzvmyuSHYrf0tLNswOeAUMaBjWbXoABhQ/JQkZr12_N5k.jpg',
		alt: "Bar's Interior",
		businessSlug: 'aloft',
	},
	{
		src: 'https://sun9-14.userapi.com/B7b6xlP6ru_wlS3Wt-sGay7QG7NWs5t9_1z0qg/OkEmHx-sek8.jpg',
		alt: "Bar's Interior",
		businessSlug: 'aloft',
	},
	{
		src: 'https://sun9-75.userapi.com/X_yHjXIl-srp1pulPTdqDr3tXioYz5YLM2Stjw/miqd1UgJ1gE.jpg',
		alt: "Bar's Interior",
		businessSlug: 'aloft',
	},
	{
		src: 'https://sun9-64.userapi.com/mGXlBkeLIe7pLXN4-aD5C_TZNJ9c0UVM5TzwKw/tg_tBMjAnlg.jpg',
		alt: "Bar's Atmosphere",
		businessSlug: 'aloft',
	},
	{
		src: 'https://sun9-20.userapi.com/h4s50EJBbkyMQfs4brQEv9QV1FLiTVtLnTzlcA/VARe1bijklw.jpg',
		alt: "Bar's Atmosphere",
		businessSlug: 'aloft',
	},
	{
		src: 'https://sun9-3.userapi.com/R68mTvRwrmusQqwqd5OeIoclpsKKWgqtVLkBoQ/ifmZJYRzts8.jpg',
		alt: "Bar's Atmosphere",
		businessSlug: 'aloft',
	},
	{
		src: 'https://sun9-69.userapi.com/XbGRuLrOMOfZv9zfC4IwyKTcAMghZRG8qyf0ow/TmVvlJr4sFc.jpg',
		alt: "Bar's Logo",
		businessSlug: 'brutal',
	},
	{
		src: 'https://sun9-39.userapi.com/8OC9mf-e8fTjynzDWicYFpK6ou9oFg3E-WhEAA/RIdtcx5hPfM.jpg',
		alt: "Bar's Interior",
		businessSlug: 'brutal',
	},
	{
		src: 'https://sun9-50.userapi.com/Yj3k6b9wjGSAJT1MaZbTAwz09AKh8ph0RW4x5A/xHRBPlpAsaM.jpg',
		alt: "Bar's Interior",
		businessSlug: 'brutal',
	},
	{
		src: 'https://sun9-52.userapi.com/d_THuZ7P8pAr07ML6QsWMYdMqWEKR9vg4cLjjQ/FnMRkrxrb1E.jpg',
		alt: "Bar's Guests",
		businessSlug: 'brutal',
	},
	{
		src: 'https://sun9-26.userapi.com/kvTYvoGdrp4qH762KrQ0RYyLwQqoUmsJ_xrjhQ/vZY48e0LTOA.jpg',
		alt: "Bar's Atmosphere",
		businessSlug: 'brutal',
	},
	{
		src: 'https://sun9-23.userapi.com/Zh1zm0c1LtCNtixhdc4Hkqp9ojsZvYt0ex1umg/PZfi42kVZ84.jpg',
		alt: "Bar's Interior",
		businessSlug: 'bsb',
	},
	{
		src: 'https://sun9-70.userapi.com/EtKKdibJ0wJXU9ZGzuKPO5ZrJGAxXEXgDZsGPQ/1HNfTMfBvBA.jpg',
		alt: "Bar's Interior",
		businessSlug: 'bsb',
	},
	{
		src: 'https://sun9-49.userapi.com/W0bQSE7epFpYrsXyIqCL0ya9z7HZhNIlE8rCVw/KY3CL3fYxuM.jpg',
		alt: "Bar's Staff",
		businessSlug: 'bsb',
	},
	{
		src: 'https://sun9-27.userapi.com/Ap7X8LC93D7_rj9vBfrAbFR0ioB9X79-q-N7XQ/IwWabqJo3-o.jpg',
		alt: "Bar's Staff",
		businessSlug: 'bsb',
	},
	{
		src: 'https://sun9-28.userapi.com/8giTrUhg-tzvG0YCykv5rU8DOLKqOJKvAL3mKg/sJ3d_cXgzJE.jpg',
		alt: "Bar's Footage",
		businessSlug: 'bsb',
	},
	{
		src: 'https://sun9-13.userapi.com/yVf_AMBDig7mThhw2VgR-xgFVJOeBU9h3-KN5g/d5PcaKtkgbY.jpg',
		alt: "Bar's Food",
		businessSlug: 'bsb',
	},
	{
		src: 'https://sun9-71.userapi.com/CoRG9WCnk0m1pKtrd9UIeGlk5hSAlkYW7mbhhg/3kEqz9UnxVI.jpg',
		alt: "Bar's Logo",
		businessSlug: 'hplace',
	},
	{
		src: 'https://sun9-27.userapi.com/nZsNbWgD6fs1XVAU2XQMhKqcT_zXA0HL1IkU9g/Fbv_JoBTG3s.jpg',
		alt: "Bar's Interior",
		businessSlug: 'hplace',
	},
	{
		src: 'https://sun9-45.userapi.com/UZRWaAOYIHMoe41SOH948Ex70PFSC2EtfYseHw/jUe7-Lx1aNc.jpg',
		alt: "Bar's Staff",
		businessSlug: 'hplace',
	},
	{
		src: 'https://sun9-27.userapi.com/xpSjMWIhZE-2rDxPm-ZpqK6VEFaG7Wn24mkaPA/xVi7pgexpW8.jpg',
		alt: "Bar's Cocktail",
		businessSlug: 'hplace',
	},
	{
		src: 'https://sun9-42.userapi.com/HIXzQB6aib-nrjYbPTUXXFsWDUbSna9YjYO8Hw/M6fbVPezMQg.jpg',
		alt: "Bar's Cocktail",
		businessSlug: 'hplace',
	},
	{
		src: 'https://sun9-34.userapi.com/JRKgLOSWcF-oxdVs2qFuz_Efweql7I7rEM52Sw/-U4JgDCzqRk.jpg',
		alt: "Bar's Food",
		businessSlug: 'hplace',
	},
	{
		src: 'https://sun9-66.userapi.com/Z8H37tGYqMzeTs6S1Sr-weqnNxwvGma3Uy81-Q/-mh0OkpfkfM.jpg',
		alt: "Bar's Interior",
		businessSlug: 'jin',
	},
	{
		src: 'https://sun9-16.userapi.com/M_r6endg7zL6C5ZK92wdBShJ5kjRXvblhrz-4g/jxFrL1HZnyQ.jpg',
		alt: "Bar's Interior",
		businessSlug: 'jin',
	},
	{
		src: 'https://sun9-38.userapi.com/uI9laLTo5Xp6M_lEgvJR-3y0WYpK7-YhOXruNg/wMlnjSgTJ2U.jpg',
		alt: "Bar's Interior",
		businessSlug: 'jin',
	},
	{
		src: 'https://sun9-25.userapi.com/e8pLDO7jY1G8LUOADcTla8HX5VtkwhHGOZ0dkg/h_UhvO0_D2s.jpg',
		alt: "Bar's Staff",
		businessSlug: 'jin',
	},
	{
		src: 'https://sun9-48.userapi.com/FhOVL3AJISskA_fQ_rbDQZyE1wPZA0ZMEIPDCg/lqI-TO7eHps.jpg',
		alt: "Bar's Food",
		businessSlug: 'jin',
	},
	{
		src: 'https://sun9-51.userapi.com/EY64Qwx3_ujaDnKrtlv2YPjWiiTViCRVyySoNg/nvHK1L8uv14.jpg',
		alt: "Bar's Drinks",
		businessSlug: 'jin',
	},
	{
		src: 'https://sun9-22.userapi.com/HSTGTooVb_z7ZQu0qcbBxraoilYgpKbMeBKwfQ/_iGU3DliWr8.jpg',
		alt: "Bar's Logo",
		businessSlug: 'light',
	},
	{
		src: 'https://sun9-16.userapi.com/mSOIEhdyf8UqJtcIeswuNciQl544m_Yy4XbIog/hlyEYBH72vY.jpg',
		alt: "Bar's Interior",
		businessSlug: 'light',
	},
	{
		src: 'https://sun9-36.userapi.com/dkBWvsZoDPHrZlYNXV2OKOf1sMfuCv3GV1oivA/O1Yh_lccpMk.jpg',
		alt: "Bar's Interior",
		businessSlug: 'light',
	},
	{
		src: 'https://sun9-7.userapi.com/q5trt5jWqg-1by-rOa33Q5N_pc2qFsuvBacXsQ/cmrzlYoDho4.jpg',
		alt: "Bar's Hookah",
		businessSlug: 'light',
	},
	{
		src: 'https://sun9-32.userapi.com/BBcu65ao_lr4yr1OeYpDLs5bOXoufaJauACpZA/cajR2Rc2hgM.jpg',
		alt: "Bar's Food",
		businessSlug: 'light',
	},
	{
		src: 'https://sun9-70.userapi.com/d8GQygKqlm_2pFdlTO2KAKNfbNUL1U4ZDRUNhQ/XuBKVBQ8mv4.jpg',
		alt: "Bar's Food",
		businessSlug: 'light',
	},
	{
		src: 'https://sun9-6.userapi.com/MTGF9PiIAvj0I6qPFhmXyU2ocJPWW0f--iRWeg/MxWvp2tQsLg.jpg',
		alt: "Bar's Interior",
		businessSlug: 'mint',
	},
	{
		src: 'https://sun9-26.userapi.com/DRPVJFoccP0zUJwdjDc7rbDqWwJmVW1iPbAxeQ/-r4JS6zlubM.jpg',
		alt: "Bar's Interior",
		businessSlug: 'mint',
	},
	{
		src: 'https://sun9-53.userapi.com/aB2DzGn8uXNSyTXYHCFrB7hdwWYHhexwEytHtQ/QjgTuDn36X4.jpg',
		alt: "Bar's Interior",
		businessSlug: 'mint',
	},
	{
		src: 'https://sun9-8.userapi.com/EEPv5SU1lgSDkmzO-McafaZxkyWnGFhM-4EPGw/n0zjv9rKhlY.jpg',
		alt: "Bar's Footage",
		businessSlug: 'mint',
	},
	{
		src: 'https://sun9-49.userapi.com/ezGyxEm8iXhKQAihtnQjapvJv_15wyeY5cLIiQ/YVSOY92F0cQ.jpg',
		alt: "Bar's Footage",
		businessSlug: 'mint',
	},
	{
		src: 'https://sun9-65.userapi.com/LOoZLlQZJn3vZ3p-bxkq6d0_LCHkJnJA47w_1g/IpBwYGrkq70.jpg',
		alt: "Bar's Footage",
		businessSlug: 'mint',
	},
	{
		src: 'https://sun9-26.userapi.com/9GQwNV7nzlw0eblvTm9nPQ-SV5ceoiSGh9dcgw/3JIfZSfXhkk.jpg',
		alt: "Bar's Interior",
		businessSlug: 'mrmld',
	},
	{
		src: 'https://sun9-16.userapi.com/X8s5ZPUnVE-Emj5TTRizYuqqPok0FW1zLK03vA/hulv3mOwiAY.jpg',
		alt: "Bar's Interior",
		businessSlug: 'mrmld',
	},
	{
		src: 'https://sun9-58.userapi.com/WMpOHkSKbSaz2Hk1SRman1MRFOK5ftfFZ1EEQA/0ka0O0HrWtM.jpg',
		alt: "Bar's Interior",
		businessSlug: 'mrmld',
	},
	{
		src: 'https://sun9-23.userapi.com/C52CvDmGP1hvJJtSSd9HXgKlahD82cH2bel1xg/cwUYCSWriEg.jpg',
		alt: "Bar's Staff",
		businessSlug: 'mrmld',
	},
	{
		src: 'https://sun9-1.userapi.com/5EMcIzt-dPq_3HJ-4jK-I0CWXX63sbkF0xWOJw/9xKifs_07Vs.jpg',
		alt: "Bar's Cocktails",
		businessSlug: 'mrmld',
	},
	{
		src: 'https://sun9-13.userapi.com/-z7vVquxurGhI7tvjaE2vpRL2-32NsmPVTKvwQ/xcqjDVl10l0.jpg',
		alt: "Bar's Cocktails",
		businessSlug: 'mrmld',
	},
	{
		src: 'https://sun9-71.userapi.com/C9qmrAWNh6bE7zdvb364ZVC0GaA_gqeg2R5pWQ/gjPOco0sb5U.jpg',
		alt: "Bar's Interior",
		businessSlug: 'office',
	},
	{
		src: 'https://sun9-71.userapi.com/L7bzwZOg6R9DN4ATw0E7755tvMAMcy-uPeVouw/S0SuffV94ZI.jpg',
		alt: "Bar's Interior",
		businessSlug: 'office',
	},
	{
		src: 'https://sun9-58.userapi.com/W3LxrYMp2MTYfnBySIbHHqOrVe2O7c26S0YI2Q/bjGnEKcBeuA.jpg',
		alt: "Bar's Interior",
		businessSlug: 'office',
	},
	{
		src: 'https://sun9-69.userapi.com/4e0JZ6yjN5Kak49BMiAgmaj7u91xeUF9hmVJAQ/42NRoomXNc8.jpg',
		alt: "Bar's Guests",
		businessSlug: 'office',
	},
	{
		src: 'https://sun9-71.userapi.com/4YMbcDH-Dx1KfaqagBst0QAQNjPlgRSbG1GfLA/HuV3Cj5XMrM.jpg',
		alt: "Bar's Guests",
		businessSlug: 'office',
	},
	{
		src: 'https://sun9-19.userapi.com/kNHgr-Y5RQm20DbRFx9Wm8IRcMJ3Bp77ZzT6ZQ/HpaCIjn95OE.jpg',
		alt: "Bar's Guests",
		businessSlug: 'office',
	},
	{
		src: 'https://sun9-73.userapi.com/mH3ZvMuTKy-4glcb-EuabSmL25NxpHw0GIYTAA/p27WZdl4hy8.jpg',
		alt: "Bar's Outside",
		businessSlug: 'pbar',
	},
	{
		src: 'https://sun9-75.userapi.com/jrrfoP9jzgqhiwGbuovWJ5Q7WZYodGk2ETaz_A/dbnexX5w7YY.jpg',
		alt: "Bar's Interior",
		businessSlug: 'pbar',
	},
	{
		src: 'https://sun9-5.userapi.com/STDnaHTtXjosKxsChopoRQKYRI0azs-QKCKUKg/O663GcICMb8.jpg',
		alt: "Bar's Interior",
		businessSlug: 'pbar',
	},
];
